...
void Timer3_Config(){
	timer_parameter_struct Timer3_InitStructure;
	rcu_periph_clock_enable(RCU_TIMER3);
	//外设时钟使能
	timer_deinit(TIMER3);
	Timer3_InitStructure.prescaler	 = 4199;
	//预分频系数
	Timer3_InitStructure.period 	 	 = 20;
	Timer3_InitStructure.alignedmode = TIMER_COUNTER_EDGE;
	Timer3_InitStructure.counterdirection = TIMER_COUNTER_UP;
	Timer3_InitStructure.clockdivision = TIMER_CKDIV_DIV1;
	Timer3_InitStructure.repetitioncounter = 0;
	timer_init(TIMER3,&Timer3_InitStructure);
